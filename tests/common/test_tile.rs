...
mod tests {
    ...
    fn test_ordering() {
        ...
        // Ord 是基于 id 实现的，所以它们比较结果应该是相等
        use std::cmp::Ordering;
        assert_eq!(m5.cmp(&r5m), Ordering::Equal);
        // 但 PartialEq 是不同的
        assert_ne!(m5, r5m);

        // 排序测试
        let mut tiles = vec![p1, m9, east, m1, r5m, white, m5];
        // 预期排序结果 (基于 ID): 1m, 5m, 0m(R), 9m, 1p, E, P
        // 注意：因为 m5 和 r5m 的 id 相同，它们的相对顺序在 sort 中是不稳定的
        // 但它们应该排在一起。
        tiles.sort();
        ...
        // m5 和 r5m (id 4)
        assert!((tiles[1] == m5 && tiles[2] == r5m) || (tiles[1] == r5m && tiles[2] == m5));
